...
class WMbed:
    ...
    def from_bytearrays(self, image_bytearray, watermark_bytearray, store_watermark_copy=True):
        self.image = cv2.imdecode(np.frombuffer(image_bytearray, np.uint8), cv2.IMREAD_COLOR)
        self.watermark = cv2.imdecode(np.frombuffer(watermark_bytearray, np.uint8), cv2.IMREAD_COLOR)
        if store_watermark_copy:
            self.original_watermark = np.copy(self.watermark)
